...
# GLM in R: the short version

Maybe you already know what a GLM is, and you just want to know how to run it in R. Maybe you already completed this tutorial and just want to refresh your memory. In that case, this section is probably all you need.

For this quick demonstration, we'll generate data with a log link function and Poisson error distribution. We'll pretend that our dependent variable is the number of times a tweet is retweeted, and the independent variable is how funny the tweet was on a scale from 1 to 10.
We know that our dependent variable `retweets` is a count variable, and we think the number of retweets will be higher for `funny` tweets. We don't think this relation will be linear, because tweets that have been retweeted reach a wider audience and as such become more likely to be retweeted more. 
...
To fit a GLM model, R has the `glm` function. This works just like the regular `lm` function for linear models. The only difference is that the error distribution and link function need to be specified. For this we use the `family` argument, which takes a family object. For Poisson regression the code is as follows.

```{r}
m = glm(retweets ~ funny, d, family=poisson(link = 'log'))
```

We can inspect the model with the `summary` function, but there are also packages such as `sjPlot` that print the data in a nice table and with additional information such as pseudo R measures. 

```{r}
tab_model(m)
```

`sjPlot` also has a nice function for plotting the data with the fitted values. 

```{r glm_tldr_plot, fig.align='center', fig.height=3, fig.width=5}
plot_model(m, type = 'pred', show.data=T)
```

Here we see how the GLM fits the data. Note the exponential increase (due to the log link function) and the increase in variance along with x (due to the Poisson distribution)

You can consult the help page for `?family` For the available distributions and link functions.
...
As the name implies, Generalized Linear Models are a generalization of ordinary linear regression models. While ordinary linear regression is a powerful tool that we all know and love, there are some cases where it cannot (or at least should not) be applied. For instance, if the dependent variable is binary (values of 0 or 1), then a logistic regression model (which is one form GLM) is more appropriate.
...
Now we can again make the scatterplot to see how the new predictions fit the data.
However, we cannot use the `abline` function for this, because this can only add straight lines.
Instead, we'll calculate the predictions for the possible values of petal length.

```{r glm_glm_on_binary, fig.align='center', fig.height=3, fig.width=5}
## create sequence of x values from lowest to highest in our data
x_min_to_max = seq(min(x), max(x), by = 0.1)
## predict y for these values
y_hat = predict(m_glm, newdata = data.frame(x = x_min_to_max), type = 'response')
## create the scatterplot
plot(x, y, bty='n')
## add regression line
lines(x_min_to_max, y_hat, col='green')
```
...
In this tutorial we'll focus on the interpretation of logistic regression, which introduces you to key concepts that are the same or similar in other GLMs. 
...
We can fit the `glm` as before. We now only add `data = d` because our variables our now in a data.frame. Also, this time we do not specify the "logit" link (which is the default). This is the same model as above, but it's good to know that the following syntax also works.
...
We can also use the `plot_model` function to visualize various aspects of the model. Here we visualize the predicted values (i.e. average probabilities) for different independent variables.

```{r gtd_example_plot , fig.align='center', fig.height=3, fig.width=5}
...
Carefully think about what `3.5 times greater` means here in terms of probability. Consider a student who, given a certain amount of studying and alcohol use, has a 60\% probability of passing the test without taking the selftest. If he/she would take the selftest, we would expect the odds to be about 3 times greater, so what would the change in probability be? We can calculate this by transforming this probability to odds, multiplying by the odds ratio, and then tranforming the new odds back to probabilities.
...
So for this person the probability of passing the test is 0.912. To verify that our calculation is correct, we can also use the predict function to get this probability. Given the model, and a dataset with the cases for which we want to predict the response, we compute this as follows.
...
You often see that multiple models are reported with increasing complexity (i.e. more variables added). 
Making a regression table with stepwise models is easy with the tab_model function.
...
```{r}
m_NULL = glm(passed_test ~ 1, data=d, family= binomial)
m_1 = glm(passed_test ~ hours_studied + selftest, data=d, family= binomial)
m_2 = glm(passed_test ~ hours_studied + selftest + alcohol, 
        data=d, family=binomial)

tab_model(m_NULL, m_1, m_2)
```
...
```{r}
anova(m_NULL, m_1, m_2, test = 'Chisq')
```
...
```{r, glm_visual_exp2, fig.align='center', fig.height=3, fig.width=5}
...
```{r, glm_visual_exp3, fig.align='center', fig.height=3, fig.width=5}
nice_plot(glm(y~x, family=poisson('log')))
```

This time we do see that the confidence interval becomes wider as `x` increases. 
...
```{r, glm_visual_exp4, fig.align='center', fig.height=3, fig.width=5}
