...
This is done by *rendering* the text and data analysis outcomes (and visualizations!) into a single document.
...
You should see a button in R Studio called 'Render' (in the top of this pane).
Go ahead and click that button! R should then run all the code chunks, and create an html document containin both the text, the code, and the results of running the code. 
...
Try changing the chunk header above to `{r import, message=FALSE}`, render again, and see if you understand the difference. 
You can also add `warning: false` to the execution options in the top, which will suppress all messages and warnings in the document
...
+ Make sure you don't include and `install.packages` or `View` commands as these cannot be used while rendering.