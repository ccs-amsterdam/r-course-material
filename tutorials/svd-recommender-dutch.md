...
-   [Een makkelijkere functie voor
    SVD](#een-makkelijkere-functie-voor-svd)
...
Zoals je in bovenstaande code kan zien, werkt `svd` met matrices in
plaats van data frames (of tibbles). Om het wat makkelijker te maken
hiermee te werken staat hieronder een functie die de output van `svd`
wat opschoont en omzet naar tibbles:
...
Deze functie staat ook online op
[Github](https://gist.github.com/vanatteveldt/a8ad49918036937ea60166b923e4c041)
en je kan het direct in R inladen met:
...
``` r
m = run_svd(reviews_matrix, ndimensions=8)
```
...
Nu kunnen we gemakkelijk kijken naar hoe de films in deze dimensies
passen door de item-values te bekijken:
...
Hetzelfde kunnen we doen met de voorspellingen. We kunnen de voorspelde
user-review matrix bekijken door de voorpellingen naar een wide formaat
te pivoten:
