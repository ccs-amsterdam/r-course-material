...
Now, we can create a dtm as normal. We won't apply any preprocessing steps (other than the default lowercasing), because dictionaries tend to have full (i.e. not-stemmed) words, and sometimes contain punctuation such as emoticons.
...
```
```{r, echo=F}
validation = tibble(doc_id=sample_ids, manual_sentiment = rep(c(1,1,0,-1, -1), 10)) %>% inner_join(result)
```
