...
In particular, we will show how [tidymodels](machine_learning.md) can be used
...
As you can see, there's the star rating (dichotomised to positive and negative), a summary text, the full review text, and the ID of the reviewer and the product (ASIN). 
...
Thus, it is a good idea to now do some hyperparameter tuning for the regularization penalty and other parameters (see the section on tuning below). For more information, take a look at the [machine learning handout](machine_learning.md) and/or the [tune documentation](https://tune.tidymodels.org/)
...
Rather than using word frequencies (i.e. DTM columns) as features, we can also use word embedding dimensions. 
First, let's download these embeddings using the textdata package:
...
Now, you can manually join these embeddings with the tokens, and use these columns as numerical predictors. 

To make this a bit easier, `textrecipes` contains a step to do this. 
Note that this recipe seems to assume that there is a single column labeled `text`,
so we merge the summary and reviewText columns:
...
So you can see, for every document it computed the mean dimension loading 
...
Did it acutally perform better in this case? Why would that be the case? Can you think of ways to improve this?
