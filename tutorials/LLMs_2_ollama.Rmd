...
Before we start talking to Ollama, we'll first need to download a model. This is as easy as now telling Ollama (in our terminal, or in the shell that Docker opened) to `run` a model. You can find an overview of the available models on the [Ollama website](https://ollama.ai/library). For now we'll run a popular model called `llama3.1`, which should run fine on most devices. Enter the following command:
...
Since we already have the Ollama server running, and have already downloaded the llama3.1 model, you can get started right away. If not, you can follow [rollama's instructions](https://github.com/JBGruber/rollama) for running the Docker container.
...
Here is one example from the vignette.
